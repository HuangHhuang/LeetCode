...
function build (nodes) {
    ...
    for (let i = 0; i < nodes.length; i++) {
        let {pidStr = 'pid', idStr = 'id', childrenStr = 'children'} = nodes[i];
        listOjb[nodes[i][idStr]] = nodes[i]
    }
    // 根据pid来将数据进行格式化
    for (let j = 0; j < nodes.length; j++) {
        // 判断父级是否存在
        let haveParent = listOjb[nodes[j][pidStr]] 
        if (haveParent) {
            // 如果有没有父级children字段，就创建一个children字段
            !haveParent[childrenStr] && (haveParent[childrenStr] = [])
            // 在父级里插入子项
            haveParent[childrenStr].push(nodes[j])
        } else {
            // 如果没有父级直接插入到最外层
